...
def main():
    ...
    # Load campaign data
    state = (CAMP_DIR / "state.md").read_text(encoding="utf-8")
    npcs = (CAMP_DIR / "npcs.md").read_text(encoding="utf-8")
    npcs_full = ((CAMP_DIR / "npcs-full.md").read_text(encoding="utf-8")
                 if (CAMP_DIR / "npcs-full.md").exists() else "")
    npc_full = extract_target_npc(npcs_full) or "(target NPC entry not found in npcs-full.md)"

    session_log = (CAMP_DIR / "session-log.md").read_text(encoding="utf-8")
    ...
    pathlib.Path("/tmp/replay-summary.json").write_text(
        json.dumps({"results": results, "n_per_condition": args.n}, indent=2), encoding="utf-8")
